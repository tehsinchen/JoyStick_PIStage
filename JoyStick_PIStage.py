...
def centralize_pos_piezo():
    with GCSDevice() as e518:
        e518.ConnectUSB(serialnum='120027848')
        pitools.startup(e518, stages=None, refmodes=None)
        rangemin = list(e518.qTMN().values())
        rangemax = list(e518.qTMX().values())
        ranges = zip(rangemin, rangemax)
        centeral_pos = []
        for i, pos in enumerate(ranges):
            centeral_pos.append(int((pos[0]+pos[1])/2))
        e518.MOV(e518.axes, centeral_pos)
        pitools.waitontarget(e518)
        positions = e518.qPOS(e518.axes)
        print(positions)


def motor_curr_pos():
    with GCSDevice() as c867:
        c867.ConnectUSB(serialnum='0117025363')
...
class JoyStick(tk.Frame):
    ...
    def set_stage(self):
        ...
        if self.pressed:
            ...
            target_x = cur_pos[0] + unit*increment_x
            target_y = cur_pos[1] + unit*increment_y
